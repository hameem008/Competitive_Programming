...
private:
    ...
    T mxquery(ll v, ll tl, ll tr, ll l, ll r)
    {
        if (l > r)
            return 0;
        if (l == tl && r == tr)
            return prefix[v];
        ll tm = (tl + tr) / 2;
        T a = mxquery(v * 2, tl, tm, l, min(r, tm));
        T b = mxquery(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r);
        return max(a, query(l, min(r, tm)) + b);
    }
...
public:
    ...
    T mxquery(ll l, ll r)
    {
        return mxquery(root, tla, tra, l, r);
    }
