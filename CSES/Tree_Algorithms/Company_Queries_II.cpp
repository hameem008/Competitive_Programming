...
ll time_ = 0;
void calc_time(ll vertex, vector<vector<ll>> &graph, vector<bool> &isVisited)
{
    isVisited[vertex] = true;
    time_++;
...
void solve(int I, int T)
{
    ...
    vector<ll> v;
    bin_lift_func(1, graph, isVisited, v, 0);
    isVisited.assign(n + 1, 0);
    calc_time(1, graph, isVisited);
    for (i = 1; i <= q; i++)
    {
        cin >> a >> b;
        cout << least_common_ancestor(a, b) << endl;
    }
    // solve ends
}
