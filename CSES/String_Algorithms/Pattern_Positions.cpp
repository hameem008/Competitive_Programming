...
bool pf_first(int mid, int idx, string &ptrn, string &str)
{
...
int first_occurance(int lo, int hi, int idx, string &ptrn, string &str)
{
    int mid;
    while (hi - lo > 1) // FFFFTTT find first true
    {
        mid = (hi + lo) / 2;
        if (pf_first(mid, idx, ptrn, str))
            hi = mid;
        else
            lo = mid + 1;
    }
    if (pf_first(lo, idx, ptrn, str))
        return lo;
    else if (pf_first(hi, idx, ptrn, str))
        return hi;
    else
        return -1;
}

bool pf_last(int mid, int idx, string &ptrn, string &str)
{
    int str_idx = suffix_array[mid] + idx;
    if (str_idx >= str.size())
        return true;
...
int last_occurance(int lo, int hi, int idx, string &ptrn, string &str)
{
    int mid;
    while (hi - lo > 1) // TTTTFFF find last true
    {
        mid = (hi + lo) / 2;
        if (pf_last(mid, idx, ptrn, str))
            lo = mid;
        else
            hi = mid - 1;
    }
    if (pf_last(hi, idx, ptrn, str))
        return hi;
    else if (pf_last(lo, idx, ptrn, str))
        return lo;
    else
        return -1;
}

void find_sub_string(string &sub_str, string &str)
{
    bool flag = true;
    int first_occ = 0, last_occ = suffix_array.size() - 1;
    for (int i = 0; i < sub_str.size(); i++)
    {
        int val = first_occurance(first_occ, last_occ, i, sub_str, str);
        last_occ = last_occurance(first_occ, last_occ, i, sub_str, str);
        first_occ = val;
    ...
    if (flag)
        cout << sg.query(first_occ, last_occ) + 1 << endl;
    else
        cout << -1 << endl;
}
